...
func_csm = function(a,b,structure,balls,aspect){
  if (aspect == 'necessary'){
    if (balls == 1){ #both balls go through
      if (structure == 'and'){
        out = 1
...
df.prediction = expand.grid(balls = balls,structure = structure) %>%
  ...
  rowwise() %>%
  # optimal intervention model 
  mutate(p.outcome.intervention.a = func_outcome_probability(a.intervention, b.prob, structure),
         p.outcome.intervention.b = func_outcome_probability(b.intervention, a.prob, structure),
         a.change = abs(outcome-p.outcome.intervention.a),
         b.change = abs(outcome-p.outcome.intervention.b)
         # a.change = a.change^2,
         # b.change = b.change^2
  ) %>% 
  ...
  # csm
  mutate(a.csm.necessary = func_csm(a.prob, b.prob, structure, balls, aspect = 'necessary'),
         b.csm.necessary = func_csm(b.prob, a.prob, structure, balls, aspect = 'necessary'),
         a.csm.sufficient = func_csm(a.prob, b.prob, structure, balls, aspect = 'sufficient'),
         b.csm.sufficient = func_csm(b.prob, a.prob, structure, balls, aspect = 'sufficient')
  ) %>% 
  ungroup() %>% 
  mutate(
...
model.names = c('abnormal selection', 'correspondence', 'necessity and sufficiency', 'optimal intervention')

df.betas = tibble(model = model.names,
                  beta = NA)
df.betas$beta[1] = func_fit_beta(data = df.data$percentage, cause = rep(c(1,0),each=3), alternative = rep(c(0,1),each=3))$par
df.betas$beta[2] = func_fit_beta(data = df.data$percentage, cause = df.prediction$b.correspondent, alternative = df.prediction$a.correspondent)$par
df.betas$beta[3] = func_fit_beta(data = df.data$percentage, cause = df.prediction$b.icard, alternative = df.prediction$a.icard)$par
df.betas$beta[4] = func_fit_beta(data = df.data$percentage, cause = df.prediction$b.change, alternative = df.prediction$a.change)$par
...
df.prediction = df.prediction %>% 
  mutate(abnormal_selection = func_softmax(balls == 'both balls go through', balls != 'both balls go through', df.betas$beta[1]),
         ...
         intervention = func_softmax(b.change, a.change, df.betas$beta[4])
  )
...
ggplot(df.plot,aes(x = structure,y=percentage))+
  ...
  scale_fill_manual(values = c('ball that normally gets blocked' = "#72E669",
                               'ball that normally goes through' = "#0C67E0"
                               ))+
