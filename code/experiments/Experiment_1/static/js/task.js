 ...
 *************************/
 ...
 var Instructions = function() {
	...
	if (this.trialinfo.pA == 0.2 && this.trialinfo.pB == 0.8){
		$('#fig2caption').html("Figure 2: <span style='color:#3EA333;font-weight:bold'>Ball A</span> was blocked. <span style='color:#2662E0;font-weight:bold'>Ball B</span> wasn't blocked.");
		$('#instruction_paragraph').html("Darker motion blocks are better at blocking balls than lighter ones. In the example below, there is a stronger motion block for <span style='color:#3EA333;font-weight:bold'>ball A</span> than there is for <span style='color:#2662E0;font-weight:bold'>ball B</span>.");
	}else if (this.trialinfo.pA == 0.8 && this.trialinfo.pB == 0.2){
		$('#fig2caption').html("Figure 2: <span style='color:#3EA333;font-weight:bold'>Ball A</span> wasn't blocked. <span style='color:#2662E0;font-weight:bold'>Ball B</span> was blocked.");
		$('#instruction_paragraph').html("Darker motion blocks are better at blocking balls than lighter ones. In the example below, there is a stronger motion block for <span style='color:#2662E0;font-weight:bold'>ball B</span> than there is for <span style='color:#3EA333;font-weight:bold'>ball A</span>.");
	}else{
		$('#fig2caption').html("Figure 2: <span style='color:#3EA333;font-weight:bold'>Ball A</span> wasn't blocked. <span style='color:#2662E0;font-weight:bold'>Ball B</span> was blocked.");
	}

	//structure paragraph 
	if (this.trialinfo.structure == 'disjunctive'){
		$('#structure').html("<span style='color:#3EA333;font-weight:bold'>Ball A</span> and <span style='color:#2662E0;font-weight:bold'>ball B</span> are set up in a way such that if at least one of the balls hits <b>ball E</b>, <b>ball E</b> will go through the gate.")
		if (this.trialinfo.pA == 0.2){
		...
		$('#structure').html("<span style='color:#3EA333;font-weight:bold'>Ball A</span> and <span style='color:#2662E0;font-weight:bold'>ball B</span> are set up in a way such that <b>ball E</b> only goes through the gate if it is hit by both balls.")
		if (this.trialinfo.pA == 0.2){
			$('#fig2info').html("For example, in Figure 2, only <span style='color:#2662E0;font-weight:bold'>ball B</span> hit <b>ball E</b>, and <b>ball E</b> didn't go through the gate.")
		}else{
			$('#fig2info').html("For example, in Figure 2, only <span style='color:#3EA333;font-weight:bold'>Ball A</span> hit <b>ball E</b>, and <b>ball E</b> didn't go through the gate.")
		}
	}
 ...
 *************************/
 var TestPhase = function(){
	...
	physics_world.onload = function(){
		physics_world.Start(structure = that.trialinfo.structure, block1 = false, block2 = false, time = 30,pos = position, pA = that.trialinfo.pA, pB = that.trialinfo.pB);	
	}
	...
	$('#simulate2').click(function () {
		switch(STATE.index){
			case 0:
				...
				physics_world.Start(structure = that.trialinfo.structure, block1 = false, block2 = false, time = 300,pos = position, pA = that.trialinfo.pA, pB = that.trialinfo.pB);
				// that.sliders("test");
				// $('.ui-slider-handle').hide();
				$('#sliders').remove() ;
				$('#choices').show() ;
